...
import { BookOpen, Plus, Clock, User, CheckCircle, XCircle, ArrowLeft, ArrowRight, Trophy, X } from "lucide-react";
...
const Tests = () => {
  ...
  const [newQuestions, setNewQuestions] = useState<{ question: string; a: string; b: string; c: string; d: string; correct: string; explanation: string }[]>([
  ...
  const startTest = async (test: Test) => {
    ...
    if (error || !data?.length) {
      toast({ title: "Nem sikerült betölteni a kérdéseket", variant: "destructive" });
      return;
    }
    setTakingTest(test);
    setQuestions(data);
  ...
  const handleCreateTest = async () => {
    ...
    const validQs = newQuestions.filter((q) => q.question && q.a && q.b && q.c && q.d);
    if (validQs.length === 0) {
      toast({ title: "Legalább egy teljes kérdés szükséges!", variant: "destructive" });
      return;
    }
    ...
    const { data: test, error } = await supabase.from("tests").insert({
      ...
      grade: parseInt(newTestGrade),
      time_limit_minutes: parseInt(newTestTime),
      creator_name: user.displayName || user.username,
      is_system: false,
    }).select().single();

    if (error || !test) {
      toast({ title: "Hiba a teszt létrehozásánál", variant: "destructive" });
      return;
    }
    ...
    setCreateOpen(false);
    setNewTestTitle("");
    setNewQuestions([{ question: "", a: "", b: "", c: "", d: "", correct: "A", explanation: "" }]);
    fetchData();
  };
        ...
        <main className="container mx-auto px-4 py-8 max-w-2xl">
          {submitted ? (
            ...
            <div>
              ...
              <div className="space-y-4">
                {questions.map((question, i) => {
                  ...
                  return (
                    <motion.div key={question.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <motion.div key={currentQ} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <p className="text-muted-foreground mb-2 text-sm">Kérdés {currentQ + 1}/{questions.length}</p>
        ...
        <DialogContent className="rounded-2xl max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            {viewingQuestions.map((q, i) => {
              ...
              return (
                <div key={q.id} className={`rounded-xl border-2 p-4 ${isCorrect ? "border-success/30" : "border-destructive/30"}`}>
                  <div className="flex items-start gap-2 mb-2">
                    {isCorrect ? <CheckCircle className="w-4 h-4 text-success mt-0.5" /> : <XCircle className="w-4 h-4 text-destructive mt-0.5" />}
                    <span className="font-semibold text-sm">{i + 1}. {q.question}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-1.5 ml-6 text-sm">
                    {labels.map((l, oi) => {
                      const isRight = q.correct_answer === l;
                      const isSel = selected === l;
                      let cls = "";
                      if (isRight) cls = "text-success font-semibold";
                      else if (isSel) cls = "text-destructive line-through";
                      return <span key={l} className={cls}>{l}) {opts[oi]} {isRight && "✓"} {isSel && !isRight && "✗"}</span>;
                    })}
                  </div>
                  {!isCorrect && q.explanation && (
                    <p className="mt-2 ml-6 text-xs bg-primary/5 rounded-lg p-2"><span className="font-semibold text-primary">Magyarázat:</span> {q.explanation}</p>
                  )}
                </div>
              );
            })}
        ...
        <DialogContent className="rounded-2xl max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-border pt-4">
              <h3 className="font-bold mb-3">Kérdések</h3>
              {newQuestions.map((q, qi) => (
                <div key={qi} className="bg-muted/30 rounded-xl p-4 mb-4">
                  ...
                  <Input value={q.question} onChange={(e) => { const nq = [...newQuestions]; nq[qi].question = e.target.value; setNewQuestions(nq); }}
                    className="mb-2 rounded-xl" placeholder="Kérdés szövege" />
                  <div className="grid grid-cols-2 gap-2">
                    {(["a", "b", "c", "d"] as const).map((opt) => (
                      <Input key={opt} value={q[opt]} onChange={(e) => { const nq = [...newQuestions]; nq[qi][opt] = e.target.value; setNewQuestions(nq); }}
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <div>
                      ...
                      <Select value={q.correct} onValueChange={(v) => { const nq = [...newQuestions]; nq[qi].correct = v; setNewQuestions(nq); }}>
                    ...
                    <div>
                      ...
                      <Input value={q.explanation} onChange={(e) => { const nq = [...newQuestions]; nq[qi].explanation = e.target.value; setNewQuestions(nq); }}
                        className="rounded-xl mt-1" placeholder="Miért ez a helyes?" />
                    </div>
                  </div>
                </div>
              ))}
              <Button variant="outline" onClick={() => setNewQuestions((qs) => [...qs, { question: "", a: "", b: "", c: "", d: "", correct: "A", explanation: "" }])}
